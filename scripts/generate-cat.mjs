// Cute chibi cat — big round head, tiny body, smooth high-poly
import fs from 'fs';
...
function sphere(r, ws, hs, scaleY = 1) {
  const p = [], n = [], idx = [];
  for (let y = 0; y <= hs; y++) {
    for (let x = 0; x <= ws; x++) {
      const u = x / ws, v = y / hs;
      const th = u * Math.PI * 2, ph = v * Math.PI;
      const px = -r * Math.cos(th) * Math.sin(ph);
      const py = r * Math.cos(ph) * scaleY;
      const pz = r * Math.sin(th) * Math.sin(ph);
      p.push(px, py, pz);
...
// === CHIBI CAT — big head, tiny body ===

// Head — big round sphere (the star of the show)
parts.push({ geo: sphere(0.75, 24, 20), pos: [0, 1.15, 0], color: w });

// Ears — rounded cones, cuter angle
parts.push({ geo: cone(0.16, 0.38, 12), pos: [-0.38, 1.9, 0], color: w, rot: [0.1, 0, 0.3] });
parts.push({ geo: cone(0.16, 0.38, 12), pos: [0.38, 1.9, 0], color: w, rot: [0.1, 0, -0.3] });

// Body — short chubby ellipsoid
...
// Paws — little round spheres
parts.push({ geo: sphere(0.1, 10, 8), pos: [-0.18, -0.1, 0.12], color: w });
parts.push({ geo: sphere(0.1, 10, 8), pos: [0.18, -0.1, 0.12], color: w });
parts.push({ geo: sphere(0.1, 10, 8), pos: [-0.2, -0.1, -0.12], color: w });
parts.push({ geo: sphere(0.1, 10, 8), pos: [0.2, -0.1, -0.12], color: w });

// Tail — cute curl (chain of small spheres)
for (let i = 0; i < 6; i++) {
  const t = i / 5;
  const x = 0;
  const y = 0.25 + t * 0.45;
  const z = -0.38 - Math.sin(t * Math.PI * 0.8) * 0.25;
  const r = 0.06 - t * 0.015;
  parts.push({ geo: sphere(r, 8, 6), pos: [x, y, z], color: w });
}
...
function encodeGLB(parts) {
  ...
  const gltf = {asset:{version:'2.0',generator:'chibi-cat'},scene:0,scenes:[{nodes:parts.map((_,i)=>i)}],nodes,meshes,materials:mats,accessors:acc,bufferViews:bv,buffers:[{byteLength:off}]};
...
fs.writeFileSync(path.join(__dirname,'..','public','models','character.glb'), glb);
console.log(`Chibi cat: ${(glb.length/1024).toFixed(1)} KB, ${parts.length} parts`);
